<div class="row justify-content-center mb-5" style="gap: 6em;">
    @foreach ($disasterDonations as $disasterDonation)
        @php
            $percentage = ceil($disasterDonation->amount / $disasterDonation->target * 100);
            $modalId = 'donate-fund-' . $disasterDonation->id;
        @endphp
        <div class="col-md-3 shadow rounded-4 py-2 border">
            <div class="campaign-card">
                <div class="p-3">
                    @if ($disasterDonation->image)
                        <img class="object-fit-cover w-100 rounded-3" src="data:image/png;base64,{{ $disasterDonation->image }}" alt="Campaign Image" style="width: 300px; height:200px;">
                    @else
