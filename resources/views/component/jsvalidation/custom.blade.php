      ...
      $("<?= $payload['selector'] ?>").each(function() {
         let validateOptions = {
            ...
            errorPlacement: function(error, element) {
               if (element.hasClass('select2-hidden-accessible')) {
               ...
               } else if (element.parent().hasClass('filepond--root')) {
                  // filepond before upload file
                  let filepondRoot = element.parent();
                  let filepondPanelRoot = filepondRoot.find('.filepond--panel-root');
                  filepondPanelRoot.css('border', '2px solid #C44E47')

                  error.insertAfter(element.parent());
               } else if (element.parent().parent().hasClass('filepond--root')) {
                  // filepond after upload file
                  let filepondRoot = element.parent().parent();
                  ...
                  filepondFile.css('background-color', '#d8544e')
